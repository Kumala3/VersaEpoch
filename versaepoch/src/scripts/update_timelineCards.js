...
function addNanoidsToCards(cards) {
  return cards.map((card) => ({
    ...card,
    short_description: card.short_description.replace(/\n\s*/g, ' ').trim(),
    full_description: card.short_description.replace(/\n\s*/g, ' ').trim(),
    id: nanoidLowercase(),
  }));
}

const updatedCards = addNanoidsToCards(timelineCards);

let output = 'export const timelineCards: TimelineCardData[] = [\n';
...
updatedCards.forEach((card, index) => {
  ...
  output += `type: ${JSON.stringify(card.type, null, 0)},\n`;
  output += `date: "${card.date}",\n`;
  output += `year: "${card.year}",\n`;
  output += `sources: ${JSON.stringify(card.sources, null, 2)},\n`;
  output += " },";
});
...
try {
  fs.writeFileSync('./update_chatgpt_timeline_cards.txt', output, 'utf-8');
  console.log('File saved successfully as ./update_chatgpt_timeline_cards.txt');
} catch (error) {
