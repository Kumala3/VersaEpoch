...
function convertToCSV(cards) {
  ...
  cards.forEach((card) => {
    ...
    const escapedValues = [
      ...
      `"${card.chatbot}"`,
      `"${card.title}"`,
      `"${card.short_description.replace(/"/g, '""')}"`,
      `"${card.full_description.replace(/"/g, '""')}"`,
      `"${JSON.stringify(card.type).replace(/"/g, '""')}"`,
      `"${card.date}"`,
      `"${card.year}"`,
      `"${JSON.stringify(card.sources).replace(/"/g, '""')}"`,
    ];
...
const csvData = convertToCSV(timelineCards);

try {
  fs.writeFileSync('./timeline_cards.csv', csvData, 'utf-8');
